...
export function BrandPicker({
  ...
  const searchResults = useMemo(() => {
    ...
    return q.length < 2
      ? []
      : models.filter((m) => m.name.toLowerCase().includes(q)).slice(0, 8);
  }, [query, models]);
  ...
  const orderedParents = [
    ...PARENT_BRAND_ORDER.filter((key) => parentGroups.has(key)),
    ...Array.from(parentGroups.keys()).filter(
      (key) => !PARENT_BRAND_ORDER.includes(key),
    ),
  ];
  const closeSearch = () => {
      ...
      >
        ...
        <div className={styles.searchField}>
          ...
          <input
            ...
            autoComplete="off"
            value={query}
            placeholder="Fx iPhone 15 Pro eller Galaxy S24"
        ...
        <p id={id + "-hint"} className={styles.hint}>
          Skriv mindst 2 tegn, eller vælg et mærke nedenfor.
        </p>
        {showResults && (
          <nav
            id={id + "-results"}
      ...
      {selectedParent ? (
        <div>
          ...
          <div className={styles.brandGrid}>
            {(parentGroups.get(selectedParent) ?? []).map((brand) => (
              <Link
                className={styles.brandCell}
                key={brand.id}
              ...
              >
                ...
                <span>
                  ...
                  <small>{DEVICE_TYPE_LABELS[brand.device_type]}</small>
                </span>
                <span aria-hidden="true">→</span>
              </Link>
            ))}
        ...
        <div className={styles.brandGrid}>
          {orderedParents.map((key) => {
            ...
            const content = (
              <>
                {meta?.logo ? (
                  <Image src={meta.logo} alt="" width={40} height={40} />
                ) : (
                  <span className={styles.deviceIcon}>
                    {DEVICE_TYPE_ICONS[collections[0].device_type]}
                  </span>
                )}
                <strong>{name}</strong>
                <span aria-hidden="true">→</span>
            ...
            return collections.length === 1 ? (
              ...
              <button
                key={key}
                ref={(node) => {
                  if (node) brandRefs.current.set(key, node);
                  else brandRefs.current.delete(key);
                }}
                type="button"
                className={styles.brandCell}
