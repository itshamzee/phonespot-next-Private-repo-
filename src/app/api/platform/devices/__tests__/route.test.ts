...
//
// Route-level test: POST /api/platform/devices (device intake) writes real
// inventory rows, so it must be staff-gated. This exercises the auth wiring
// (401 with no row written vs 201 for staff) through the real handler, with
// Supabase and requireStaff mocked out.
...
function makeRequest(body: unknown, headers: Record<string, string> = {}) {
  return new Request("http://localhost/api/platform/devices", {
    method: "POST",
...
describe("POST /api/platform/devices", () => {
  ...
  it("creates the device for a logged-in staff request and logs the real actor", async () => {
    ...
    expect(insertedDevices[0].status).toBe("intake");

    // The activity log must record who actually created the device, not
    // a client-suppliable _actorId field or the literal string "system".
    expect(insertedActivity).toHaveLength(1);
    expect(insertedActivity[0].actor_id).toBe(staffId);
  });

  it("ignores a client-supplied _actorId and always records the authenticated staff member", async () => {
    const staffId = "44444444-4444-4444-8444-444444444444";
    mockStaff = { id: staffId, role: "employee", name: "Test Staff", email: "staff@phonespot.dk" };
    const res = await POST(
      makeRequest({ ...VALID_BODY, _actorId: "spoofed-actor" }, { authorization: "Bearer valid-token" }),
    );
