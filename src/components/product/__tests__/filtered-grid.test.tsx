...
describe("FilteredGrid", () => {
  ...
  it("interleaves editorial cards without inflating counts or crowding a narrowed result", async () => {
    ...
    const panel = screen.getByRole("complementary", { name: "Produktfiltre" });
    fireEvent.change(within(panel).getByLabelText("Maksimumspris"), { target: { value: "2500" } });
    expect(within(result).getAllByRole("link")).toHaveLength(4);
    expect(screen.getAllByText("2 modeller").length).toBeGreaterThan(0);
    fireEvent.change(within(panel).getByLabelText("Maksimumspris"), { target: { value: "1500" } });
    await waitFor(() => expect(within(result).getAllByRole("link")).toHaveLength(1));
    fireEvent.change(within(panel).getByLabelText("Maksimumspris"), { target: { value: "500" } });
  ...
  it("uses one shared filter state for the mobile drawer and desktop panel", async () => {
    ...
    fireEvent.change(within(dialog).getByLabelText("Maksimumspris"), {
      target: { value: "2000" },
    });
  ...
  it("sorts by the selected meaning and resets to the complete catalogue", async () => {
    ...
    fireEvent.change(within(panel).getByLabelText("Maksimumspris"), {
      target: { value: "1500" },
    });
