"use client";

type SizeSelectorProps = {
  sizes: string[];
  selectedSize: string;
  label?: string;
  onSelect?: (size: string) => void;
};

export function SizeSelector({ sizes, selectedSize, label = "Størrelse", onSelect }: SizeSelectorProps) {
  if (sizes.length <= 1) return null;

  return (
    <div>
      <p className="mb-3 text-sm font-semibold text-charcoal">{label}</p>
      <div className="flex flex-wrap gap-2">
        {sizes.map((size) => {
          const isSelected = size === selectedSize;
          return (
            <button
              key={size}
              type="button"
              onClick={() => onSelect?.(size)}
              className={`rounded-xl border-2 px-5 py-2.5 text-sm font-semibold transition-colors ${
                isSelected
                  ? "border-green-eco bg-green-pale text-charcoal"
                  : "border-soft-grey bg-white text-charcoal hover:border-green-eco/40"
              }`}
            >
              {size}
            </button>
          );
        })}
      </div>
    </div>
  );
}
