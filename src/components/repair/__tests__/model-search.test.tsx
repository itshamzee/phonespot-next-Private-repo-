...
describe("repair model search", () => {
  it("opens matching routes with keyboard, navigates results and closes with Escape", () => {
    render(
      <BrandPicker brands={brands} models={models} basePath="/reservedele" />,
    );
    ...
    expect(
      screen.queryByRole("navigation", { name: "Søgeresultater" }),
  ...
  it("offers help for missing matches and can clear a search", () => {
    ...
    expect(screen.getByRole("link", { name: /Få hjælp/ })).toHaveAttribute(
      "href",
      "/kontakt",
    );
  ...
  it("keeps grouped brand destinations and returns to the brand list", () => {
    ...
    expect(screen.getByRole("link", { name: /iPad/ })).toHaveAttribute(
      "href",
      "/reservedele/ipad",
    );
  ...
  it("provides a labelled model filter, reset and help when models are missing", () => {
    ...
    expect(
      screen.getByRole("textbox", { name: "Søg efter model" }),
    ).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Få hjælp/ })).toHaveAttribute(
      "href",
      "/kontakt",
  ...
  it("filters models without treating an unavailable zero price as free", () => {
    ...
    fireEvent.change(screen.getByRole("textbox", { name: "Søg efter model" }), {
      target: { value: "Pro" },
    });
    expect(screen.getAllByRole("link")).toHaveLength(1);
    expect(screen.getByRole("link")).toHaveAttribute(
      "href",
      "/reparation/iphone/iphone-15-pro",
    );
